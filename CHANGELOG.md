...
## [1.11.27+4008] - 2026-09-17 — 同一 Release 同时提供加密版与免卡密版

**一句话**：正式发布同时生成 `app-release.apk`（默认卡密版）和 `app-unlocked-release.apk`（免卡密版），两者名称清楚区分且使用同一正式签名。

- 加密版继续执行设备绑定卡密、到期复核与剩余时长显示，应用内自动更新仍优先下载该包。
- 免卡密版通过编译期常量关闭授权闸门，启动后直接进入主页，设置页不显示授权区域。
- 两种包保持相同 applicationId 和正式签名，可互相覆盖安装并保留应用数据。
- GitHub Actions 在发布前继续验证卡密逻辑、个人更新源、正式签名 Secrets 和 R8 配置。

---
## [1.11.26+4007] - 2026-09-16 — 授权到期实时复核 + 剩余时长展示

**一句话**：离线卡密在冷启动、恢复前台及持续运行期间都会复核；设置页显示剩余天数和到期日，正式包已在雷电模拟器从 v1.11.25 覆盖升级并保留激活数据。

**① 到期控制**
- 到期日当天全天有效，按设备本地日期从次日起判定过期。
- 冷启动立即复核；App 恢复前台立即复核；持续前台运行每 30 秒复核。
- 过期后自动返回卡密激活页，已进入的功能页面不再继续开放。

**② 授权信息展示**
- 设置页新增「授权」区域，显示授权状态、剩余天数和明确到期日。
- 到期当天显示「今日到期（剩余不到1天）」，支持手动刷新授权状态。

**③ 卡密兼容与验证**
- 保持与 `汉唐中医卡密器.exe` 一致的 19 位离线设备绑定卡密规则。
- `flutter test test/license_service_test.dart test/services/update_service_test.dart`：20/20 通过。
- `flutter analyze lib/main.dart lib/services/license_service.dart lib/screens/settings_screen.dart`：0 issue。

**④ 发布与安装记录**
- Git 提交：`37e88e1b752c87447a7216e6d619e09eb027be2a`；标签：`v1.11.26`。
- GitHub Actions：Build APK & Release 成功。
- 通用 APK：71,569,061 字节；SHA-256 `98A6A10E3268632F62F56BB8E5F71B95448DC93D9297A716B2E2BBFF946A49D3`。
- 签名证书 SHA-256：`5c6dcc54f92db55db5fb87546fe746573bd608151031fd504de8a287896f0db6`；APK Signature Scheme v2 验证通过。
- 雷电模拟器 index 1：`v1.11.25+4006` 使用 `adb install -r` 覆盖到 `v1.11.26+4007`，返回 `Success`；设备编号、卡密和激活数据保留，启动后直接进入主页。
...
**① 修复：正文方剂·药材链接点击无反应**（`lib/screens/markdown_doc_screen.dart`）
- 根因：`flutter_markdown` 会把 `onTapLink` 的 href 做 percent-encoding（`formula://四逆汤` → `formula://%E5%9B%9B%E9%80%86%E6%B1%A4`），旧代码只去前缀未解码 → 查库恒 `null` → 静默无跳转。
- 修复：新增顶层 `decodeMarkdownHref`（`Uri.decodeComponent` + 非法 `%` 兜底），`_onTapLink` 的 formula/herb 两分支先解码再查库。一处修复覆盖全部 Markdown 阅读页（闭门课 / 伤寒金匮 / 内经搜索 / 方剂详情「见于经典」/ 药物关联闭门课）。
- 回归：`test/markdown_link_href_test.dart` 3 例全过。

**② 精度收紧至中间档**（`lib/screens/formula_detail_screen.dart` · `clauseIntroducesFormula`）
- 命中条件收窄为二选一：①方证标题首行即该方；②条文原文（`>` 引文块）中该方之后紧接「主之」。
- 舍去「宜/与/属/可/当」等弱信号，避免把对比/加减方条文也算进来；最长优先正则避免「四逆汤」误命中「茯苓四逆汤」「通脉四逆汤」。

**② 实测计数（380 伤寒 + 453 金匮真实资源）**
- 伤寒论：桂枝汤 20→12、小柴胡汤 10→5、四逆汤 ~10→9。
- 金匮要略：桂枝汤 2、小柴胡汤 4、四逆汤 3（453 条已拆好，自动升级为条文级）。

**③ 加载与验证**
- 伤寒 380 + 金匮 453 两本书各自 `Future.wait` 并行读取，首次打开方剂详情不再卡顿。
- `test/formula_intro_test.dart`(6) + `test/classic_lecture_assets_test.dart`(3) 共 9 例全过；`flutter analyze lib` 0 error。
