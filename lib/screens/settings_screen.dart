...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  late Future<LicenseInfo> _license;

  @override
  void initState() {
    super.initState();
    _license = LicenseService.current();
  ...
  Widget _licenseTile(ColorScheme cs) => FutureBuilder<LicenseInfo>(
    ...
    builder: (context, snapshot) {
      ...
      final subtitle = info == null
          ? '正在读取授权信息…'
          : '${info.remainingLabel} · 到期日 ${info.expiryLabel}';
      return ListTile(
        leading: Icon(
          info?.isValid == true ? Icons.verified_user : Icons.lock_clock,
          color: info?.isValid == true ? cs.primary : cs.error,
        ),
        title: Text(info?.isValid == true ? '授权已激活' : '授权无效'),
        subtitle: Text(subtitle),
        contentPadding: EdgeInsets.zero,
        trailing: IconButton(
          tooltip: '刷新授权状态',
          icon: const Icon(Icons.refresh),
          onPressed: () => setState(() => _license = LicenseService.current()),
        ),
      );
    },
  );
