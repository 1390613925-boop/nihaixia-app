...
class _SettingsScreenState extends State<SettingsScreen> {
  ...
  Widget _genderTile(ColorScheme cs) => ListTile(
    ...
    subtitle: Text(
      _settings.defaultGender.isEmpty
          ? '未设置（每次询问）'
  ...
  Widget _diagnosticLevelTile(ColorScheme cs) => ListTile(
    ...
    subtitle: Text(_settings.diagnosticLevel == 'simple' ? '简单模式' : '详细模式'),
    contentPadding: EdgeInsets.zero,
    trailing: SegmentedButton<String>(
      segments: const [
  ...
  Widget _lateZiTile(ColorScheme cs, {required bool isBazi}) => Column(
    ...
    children: [
      if (isBazi) ...[
        const Text('子时口径', style: TextStyle(fontWeight: FontWeight.bold)),
        const SizedBox(height: 8),
  ...
  Widget _licenseTile(ColorScheme cs) => FutureBuilder<LicenseInfo>(
    ...
    builder: (context, snapshot) {
      ...
      final status = switch (info.result) {
        LicenseResult.valid => '已激活',
        LicenseResult.expired => '已过期',
        _ => '未激活',
      };
      final remaining = info.isPermanent
          ? '—'
          : '${info.remainingDays?.clamp(0, 99999) ?? 0} 天';
      return Card(
        ...
        child: Padding(
          ...
          child: Column(
            children: [
              _licenseRow(Icons.verified_user_outlined, '激活状态', status, cs),
              _licenseRow(Icons.event_outlined, '到期日期', info.expiryLabel, cs),
              if (!info.isPermanent)
                _licenseRow(Icons.hourglass_bottom, '剩余天数', remaining, cs),
              _licenseRow(
